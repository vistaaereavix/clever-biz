...
import { Lock, Mail, AlertCircle, X, Send } from 'lucide-react';
...
export function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!sucesso) {
      setErro('E-mail ou senha inválidos. Use admin@erp.com / admin123');
    }
    setCarregando(false);
  };
  ...
  const handleRecuperarSenha = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!emailRecuperacao) {
      setMensagemRecuperacao('Por favor, informe seu e-mail.');
      return;
    }
